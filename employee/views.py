...
class EmployeeSignUpView(CreateAPIView):
    ...
    def post(self, request, format='json'):
        ...
        if serializer.is_valid():
            ...
            check_email = Employee.objects.filter(email=email).first()
            check_username = Employee.objects.filter(username=username).first()

            if check_username:
                return Response({"responseCode": 0,
                        "responseMessage": "Email name Already Exist.",
    ...
    def post(self, request, format='json'):
        ...
        try :
            ...
            if 'email' in request.data:
                user = MyBackendWithUser.authenticate(email=request.data['email'], password=request.data['password'])
                if user is not None:
